...
#ifndef _NET_TCPSOCKETCALLS_H_
#define _NET_TCPSOCKETCALLS_H_
...
class TCPSocketCalls {
    public:
        static int bindToSocket(VNode *node, int domain, int type, int protocol);

        static int accept(VNode *socknode, struct sockaddr *addr, int *addrlen, VNode **newNode);
    ...
    private:
       ...
       static FSMount tcpSocketFakeMount;
};
